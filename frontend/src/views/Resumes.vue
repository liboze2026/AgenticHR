...
async function toggleStatus(row, targetStatus) {
  ...
  if (row.status === 'passed' && targetStatus === 'rejected') {
    try {
      await ElMessageBox.confirm(`确定将 "${row.name}" 标记为已淘汰？`, '确认操作', {
        type: 'warning', confirmButtonText: '确认淘汰', cancelButtonText: '取消',
      })
    } catch { return }
  }
  try {
...
.resume-list-item.status-rejected { border-left-color: #f56c6c; opacity: 0.7; }
