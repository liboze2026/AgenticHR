...
**不含**：简历下载（Boss 推荐牛人页详情非 PDF，靠 DOM 结构化信息；现有 chat 页 PDF 采集路径保留独立）、LLM 生成打招呼话术、候选人同意告知（话术非 AI 生成，合规 R7 降级）。
...
class ScrapedCandidate(BaseModel):
    ...
    raw_text: str = ""
    boss_current_job_title: str = ""  # Q8 对齐检查
...
- F3 不处理 Boss 列表无限滚动（T1 调研：若推荐列表是懒加载，scroll 触底加载更多；若不是，跑完视口内可见为止）。**T1 研究结论写回 spec**
...
T1. **Live DOM 探查**：MCP 登入 Boss 推荐页 → 抠 selector、确认打招呼按钮行为、滚动加载模式  
