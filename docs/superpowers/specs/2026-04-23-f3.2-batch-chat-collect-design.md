# F3.2 (原 F6): 批量聊天候选人采集 — 设计文档

**Date**: 2026-04-23
**Phase**: M3 F3.2（原 F6，接续 F3.1 手动 intake）
...
F3.1 实现了单人手动 intake（扩展逐个触发）。现需要批量从 Boss 直聘消息列表页（`/web/chat/index`）采集 X 个新候选人，并按岗位配置的学校层次/学历标准过滤。
