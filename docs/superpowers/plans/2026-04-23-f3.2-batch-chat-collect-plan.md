...
git commit -m "feat(f3.2-T1): migration 0016 — jobs.batch_collect_criteria"
```
...
git commit -m "feat(f3.2-T2): Job.batch_collect_criteria column + schema"
```
...
git commit -m "feat(f3.2-T3): POST /resumes/check-boss-ids + upload source param"
```
...
git commit -m "feat(f3.2-T4): content.js school constants + extractSchoolTier + matchesCriteria + batchCollectNew"
```
...
git commit -m "feat(f3.2-T5): popup batch collect new candidates button + wiring"
```
...
git commit -m "feat(f3.2-T6): Jobs.vue batch_collect_criteria form fields"
```
