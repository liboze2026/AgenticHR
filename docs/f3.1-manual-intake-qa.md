# F3.1 — 单人手动采集 手工 QA 步骤（原 F5，已重命名）

## 前置
1. 后端：`alembic upgrade head` → 迁移 0012 应用
2. 前端：`pnpm dev` 启动
3. 登录一个 HR 账号
...
- [ ] `edge_extension/content.js` `intake_typeAndSendChatMessage` — `.chat-input textarea` 和发送按钮
- [ ] `edge_extension/content.js` `intake_clickRequestResumeButton` — `/求简历|索要简历/` 按钮文案
- [ ] `edge_extension/content.js` `intake_checkPdfReceived` — `.attachment-card` + `data-sender` 属性
